...
完整设计和适用范围见[按键说明](docs/keymap.md)。`*` 表示依赖 Codex 版本与焦点状态；确定键长按的“引导”仍需在**回复进行中且输入框聚焦**时完成实体按键验收。当前 Codex 桌面版默认“回车发送”时，代码会在回复中把 `⌘Return` 解释为与默认排队相反的“引导”；在空闲时，同一个键只会正常发送一条新消息。

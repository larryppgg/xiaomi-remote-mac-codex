...
5. Test physical buttons against the user's current Codex version. With default Enter behavior, Command-Return is a steering candidate only while a reply is running; it is not yet a verified guarantee. If it fails, report the observed result and update the preset only with evidence.
